...
namespace AMS.DBManager
{
	...
	public partial class Reportes : System.Web.UI.UserControl
	{
		#region Atributos
		...
		protected void Cambiar_Combos(object Sender,EventArgs e)
		{
			...
			if(hijos.Count!=0)
			{
				for(int i=0;i<hijos.Count;i++)
				{
					...
					string p=DBFunctions.SingleData("SELECT SFIL_CODIGOPADRE FROM SFILTROREPORTECRYSTAL WHERE SREP_CODIGO="+Request.QueryString["CodRep"]+" AND SFIL_CODIGO="+hijos[i].ToString()+"");
			...
			try
			{
				...
				if(vistaO.Length>0){
					...
					for(int i=0;i<vistas.Length;i++)
                        ...
                        if (DBFunctions.RecordExist("select NAME from sysibm.SYSTABLES where NAME='" + vistas[i].ToString().Trim().ToUpper() + "_R';"))
...
//						string newPwd=miCripto.DescifrarCadena(password);
                        ...
                        Impr.PreviewReport2(NombreReporte, header, footer, copias, paginainicial, paginafinal, "", "", NombreReporte, TipoReporte.SelectedValue.ToString(), usuario, newPwd, Formulas, ValFormulas, lb);
		...
		protected bool IsValidSchema()
		{
            ...
            String prueba= "SELECT * FROM SREPORTECRYSTAL WHERE SREP_CODIGO="+Request.QueryString["CodRep"]+";" +
				"SELECT SFIL_TEXTO,TTIP_FILTRO,SFIL_VISTA,SFIL_CONDICION,SFIL_CODIGOPADRE,SFIL_CODIGO,SFIL_VALORAMOSTRAR,SFIL_VALORCOMBO, SFIL_LUPA FROM SFILTROREPORTECRYSTAL WHERE SREP_CODIGO ="+Request.QueryString["CodRep"]+" ORDER BY SFIL_CODIGO";
		...
		protected void ImageButton1_Click(object sender, System.Web.UI.ImageClickEventArgs e)
		{
            ...
            //string newPwd=miCripto.DescifrarCadena(password);
                ...
                string rutaArchivo = ConfigurationManager.AppSettings["PathToReports"] + NombreReporte + "_" + HttpContext.Current.User.Identity.Name.ToLower() + "." + TipoReporte.SelectedValue.ToString();
                ...
                //Version anterior, no usar, solo como referencia...
                //if (!miMail.EnviarMail(
                //        ConfigurationManager.AppSettings["EmailFrom"],
                //        tbMail.Text,
                //        ds.Tables[0].Rows[0][1].ToString(),
                //        "[Correo Enviado por AMS automaticamente. Por favor no contestar]",
                //        TipoCorreo.TEXT,
                //        ConfigurationManager.AppSettings["MailServer"],
                //        ConfigurationManager.AppSettings["PathToReports"] + NombreReporte + "_" + HttpContext.Current.User.Identity.Name.ToLower() + "." + TipoReporte.SelectedValue.ToString(),
                //        "",
                //        newPwd))
                //    lb.Text = "Correo enviado satisfactoriamente a : " + tbMail.Text;
                //else
                //    lb.Text = "Error al enviar correo. Detalles: El Servidor de correos no ha sido configurado. Contactar Administrador de sistemas.";// +miMail.Mensajes;
            }
            catch (Exception er)
            {
