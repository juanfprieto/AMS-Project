...
{
	public partial class AMS_Inventarios_SegundoConteoAutomaticoInventarioFisico : System.Web.UI.UserControl
	{
		#region Atributos
		...
		protected void btnCerrar_Click(object sender, System.EventArgs e)
		{
            ...
            string sqlTsinContar    = "SELECT dinv_tarjeta FROM dinventariofisico WHERE pdoc_codigo = '" + prefijoInventario + "' AND MINF_NUMEROINV = " + numeroInventario + " AND " +
                                      "DINV_conteo1 is null ORDER BY dinv_tarjeta;";
            ...
            else
            {
                if (dsAjusteErrado.Tables.Count > 0 && dsAjusteErrado.Tables[0].Rows.Count > 0)
                {
                    ...
                    Utils.MostrarAlerta(Response, "Tarjeta " + tarjetaAjuErrado + " con Ajuste Errado, ajuste mayor a la Existencia actual, CORRIJA EL CONTEO y repite el proceso de Segundo Conteo Automático ");
                }
                else
                {
                    resultado = InventarioFisico.SegundoConteoAutomaticoInventarioFisico(prefijoInventario, numeroInventario);
                    if(resultado.Length==0)
                        Utils.MostrarAlerta(Response, "Segundo Conteo Automático al inventario Fisico realizado Correctamente, Conteo SI ejecutado !!!");
                    else
		...
		{
			Response.Redirect("" + indexPage + "?process=Inventarios.SegundoConteoAutomarticoInventarioFisico");
		}
		#endregion
