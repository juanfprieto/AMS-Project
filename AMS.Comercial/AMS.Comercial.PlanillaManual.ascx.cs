namespace AMS.Comercial
{
	...
	/// </summary>
	public class AMS_Comercial_PlanillaManual : System.Web.UI.UserControl
	{
		#region Controles
	   ...
	   // protected System.Web.UI.WebControls.TextBox txtResponsableAnticipo;
		...
		private void InitializeComponent()
		{
			...
			this.dgrTiqueteEsps.ItemDataBound += new System.Web.UI.WebControls.DataGridItemEventHandler(this.dgrTiqueteEsps_ItemDataBound);
		...
		private void dgrEncomiendas_ItemDataBound(object sender, System.Web.UI.WebControls.DataGridItemEventArgs e)
		{
			if(e.Item.ItemType==ListItemType.Item ||e.Item.ItemType==ListItemType.AlternatingItem )
			{
				...
				//CheckBox chkAsignar = (CheckBox)e.Item.FindControl("chkAsignacion");
				//chkAsignar = false;
		...
		*/
		...
		private void VerificarPlanilla(string Opcion)
		{
			...
			//papeleria planilla?
			if(!DBFunctions.RecordExist("Select NUM_DOCUMENTO from DBXSCHEMA.MCONTROL_PAPELERIA WHERE TIPO_DOCUMENTO='M' AND FECHA_ANULACION IS NULL AND FECHA_USO IS NULL AND MNIT_RESPONSABLE='"+tiquetero+"' AND MAG_CODIGO="+agencia+" AND TDOC_CODIGO='PLA' AND NUM_DOCUMENTO="+planilla))
			{
			...
			foreach(DataGridItem dgrI in dgrTiquetes.Items)
			{
				...
				if((txtTiquete.Text.Trim().Length>0 && (ddlDestino.SelectedValue.Length>0 || txtCantidadTiquete.Text.Trim().Length>0 || txtValorTiquete.Text.Trim().Length>0)))
				{
					...
					DBFunctions.Request(dsVerifica,IncludeSchema.NO,"CALL DBXSCHEMA.VERIFICA_PAPELERIA('"+TipoDoc+"',"+numDocumento+","+agencia+");");
					int Ok = Convert.ToInt16(dsVerifica.Tables[0].Rows[0]["OK"]);				
					...
					txtResponsable.Text = Resultado;
					/*
					if(!DBFunctions.RecordExist("SELECT NUM_DOCUMENTO FROM DBXSCHEMA.MCONTROL_PAPELERIA WHERE TDOC_CODIGO='TIQ' AND FECHA_ANULACION IS NULL AND FECHA_USO IS NULL AND TIPO_DOCUMENTO='M' AND MNIT_RESPONSABLE='"+tiquetero+"' AND MAG_CODIGO="+agencia+" AND NUM_DOCUMENTO="+numDocumento+" ORDER BY NUM_DOCUMENTO FETCH FIRST 1 ROWS ONLY"))
					{
						errores+="No hay papeleria disponible para el tiquete de la línea "+fila+". ";
						errLin=true;}
					*/
					//Insertar tiquete
					if(!errLin)
					{
						...
						{
							//ANULAR
							sqlUpd.Add("INSERT INTO dbxschema.mtiquete_viaje values('TIQ',"+numDocumento+","+planilla+","+linea+",'"+ruta+"','"+tiquetero+"',"+cantidad+",'"+nitPlanillaManual+"',"+valorSeguro.ToString("0")+","+precio+","+precio*cantidad+",'"+DateTime.Now.ToString("yyyy-MM-dd")+"','A');");
							sqlUpd.Add("INSERT INTO dbxschema.MTIQUETE_VIAJE_ANULADO values('TIQ',"+numDocumento+","+ddlConceptoAnulacion.SelectedValue+",'"+fecha.ToString("yyyy-MM-dd")+"','"+tiquetero+"','"+DateTime.Now.ToString("yyyy-MM-dd")+"');");
							sqlUpd.Add("UPDATE DBXSCHEMA.MCONTROL_PAPELERIA SET NUM_ANULACION=0, COD_ANULACION="+ddlConceptoAnulacion.SelectedValue+",MNIT_ANULACION='"+nitResponsable+"', FECHA_ANULACION='"+DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")+"', FECHA_USO='"+DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")+"' WHERE TDOC_CODIGO='TIQ' AND NUM_DOCUMENTO="+numDocumento+";");
						}
						else
						{
							sqlUpd.Add("INSERT INTO dbxschema.mtiquete_viaje values('TIQ',"+numDocumento+","+planilla+","+linea+",'"+ruta+"','"+tiquetero+"',"+cantidad+",'"+nitPlanillaManual+"',"+valorSeguro.ToString("0")+","+precio+","+precio*cantidad+",'"+fecha.ToString("yyyy-MM-dd")+"','V');");
							totalTiquetes+=cantidad*precio;
							totalPasajeros+=cantidad;
						}
						//Modificar papeleria(usada)
						sqlUpd.Add("UPDATE DBXSCHEMA.MCONTROL_PAPELERIA SET FECHA_USO='"+DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")+"',MPLA_CODIGO="+planilla+" WHERE TDOC_CODIGO='TIQ' AND NUM_DOCUMENTO="+numDocumento+";");
			...
			#region Tiquetes Adicionales Prepago
			ArrayList arrTiquetesPre=new ArrayList();
			
			//Porcentaje default prepago
			double porcentajePrepago=Convert.ToDouble(ViewState["PorcentajePrepago"]);
			fila=1;
			
			foreach(DataGridItem dgrI in dgrTiqueteEsps.Items)
			{
				bool errLin=false;
				TextBox txtNumTiqueteEsp=(TextBox)dgrI.FindControl("txtNumTiqueteEsp");
				DropDownList ddlDestino=(DropDownList)dgrI.FindControl("ddlDestinoTiqueteEsp");
				//DropDownList ddlTipoTiqueteEsp=(DropDownList)dgrI.FindControl("ddlTipoTiqueteEsp");
				//TextBox txtNITVentaOtro=(TextBox)dgrI.FindControl("txtNITVentaOtro");
				//TextBox txtCantidadTiqueteEsp=(TextBox)dgrI.FindControl("txtCantidadTiqueteEsp");
				TextBox txtValorTiqueteEsp=(TextBox)dgrI.FindControl("txtValorTiqueteEsp");
				TextBox txtTotalTiqueteEsp=(TextBox)dgrI.FindControl("txtTotalTiqueteEsp");
				
				if(txtNumTiqueteEsp.Text.Trim().Length>0 || ddlDestino.SelectedValue.Length>0  || txtValorTiqueteEsp.Text.Trim().Length>0)
				{
					int cantidad=0;
					double precio=0;
					//Venta de otro?
					//	bool deOtro=ddlTipoTiqueteEsp.SelectedValue.Equals("VO");
					
					//Ruta
					string ruta=ddlDestino.SelectedValue;
					if(ruta.Length==0)
					{
						errores+="Destino de tiquete adicional no válido en la línea "+fila+". ";errLin=true;
					}

					//Número Tiquete
					try
					{
						numDocumento=(long)int.Parse(txtNumTiqueteEsp.Text.Trim());
					}
					catch
					{
						errores+="Número de tiquete  no válido en la línea "+fila+". ";
						errLin=true;
					}
					
					
					//Repetido?
					if(arrTiquetesPre.Contains(numDocumento))
					{
						errores+="El número de tiquete prepago en la línea "+fila+" ya ha sido ingresado. ";
						errLin=true;
					}
					else
						arrTiquetesPre.Add(numDocumento);
					...
					try
					{
						precio=double.Parse(txtValorTiqueteEsp.Text.Replace(",","").Trim());
						if(precio<=0)throw(new Exception());}
					catch
					{
						errores+="Precio de tiquete no válido en la línea "+fila+". ";
					...
					strActScript+="totalTicketesEsp('1','"+txtValorTiqueteEsp.ClientID+"','"+txtTotalTiqueteEsp.ClientID+"');";
					...
					if(!errLin)
					{
						if(!DBFunctions.RecordExist("SELECT MRUT_VALSUG FROM DBXSCHEMA.MRUTAS WHERE MRUT_VALMAX>="+precio+" AND MRUT_VALMIN<="+precio+" AND MRUT_CODIGO='"+ruta+"';"))
						{
							txtValorTiqueteEsp.Text=Convert.ToDouble(DBFunctions.SingleData("SELECT MRUT_VALMAX FROM DBXSCHEMA.MRUTAS WHERE MRUT_CODIGO='"+ruta+"';")).ToString("###,###,##0.##");
							errores+="Precio de tiquete adicional fuera del rango válido en la línea "+fila+", el precio ha sido remplazado. ";
							strActScript+="totalTicketesEsp('1',"+txtValorTiqueteEsp.ClientID+"','"+txtTotalTiqueteEsp.ClientID+"');";
							errLin=true;
						}
					}
					...
					{
						//Hay ruta inversa? ESTE COMENTARIO SE DEBE QUITAR CUANDO SE PONGA EN PRODUCCION LA VENTA DE TIQUETES PREPAGO...etc
						...
						{
							errores+="No hay papeleria disponible para el tiquete adicional prepago de la línea "+fila+". ";
							errLin=true;
						}
						

					if(!DBFunctions.RecordExist("SELECT NUM_DOCUMENTO FROM DBXSCHEMA.MCONTROL_PAPELERIA WHERE TDOC_CODIGO='TIQPRE' AND NUM_DOCUMENTO="+numDocumento+" AND FECHA_ANULACION IS NULL AND FECHA_USO IS NULL AND TIPO_DOCUMENTO='M' ORDER BY NUM_DOCUMENTO FETCH FIRST 1 ROWS ONLY;"))
					{
						errores+="No hay papeleria disponible para el tiquete prepago de la línea "+fila+". ";
						errLin=true;
					}
					 */
					//Verificar papeleria
					DataSet dsVerifica=new DataSet();
					string TipoDoc = "TIQPRE";
					...
					if(!errLin)
					{
						...
						sqlUpd.Add("INSERT INTO dbxschema.mtiquete_viaje_prepago values('TIQPRE',"+numDocumento+","+agencia+","+planilla+","+linea+",'"+ruta+"',NULL,NULL,NULL,'"+nitResponsableAgencia+"','"+tiquetero+"','"+nitPlanillaManual+"',0,"+precio+","+porcentajePrepagoA+","+((precio*(100-porcentajePrepagoA))/100)+",'"+fecha.ToString("yyyy-MM-dd")+"','V','"+DateTime.Now.ToString("yyyy-MM-dd")+"');");
						sqlUpd.Add("UPDATE DBXSCHEMA.MCONTROL_PAPELERIA SET FECHA_USO='"+DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")+"',MPLA_CODIGO="+planilla+",MRUT_CODIGO='"+ruta+"',PORCENTAJE_DESCUENTO = "+porcentajePrepagoA+" WHERE TDOC_CODIGO='TIQPRE' AND NUM_DOCUMENTO="+numDocumento+";");
						dgrI.Cells[0].BackColor=dgrI.Cells[1].BackColor;
						linea++;
					}
					else
		...
		#region Encomiendas
			...
			foreach(DataGridItem dgrI in dgrEncomiendas.Items)
			{
				...
				if(txtNumEncomienda.Text.Trim().Length>0 || ddlDestinoEncomienda.SelectedValue.Length>0  || txtValorEncomienda.Text.Trim().Length>0)
				{
					...
					{
						errores+="El número de encomienda/Giro en la línea "+fila+" ya ha sido ingresado. ";
						errLin=true;
					}
					else
					...
					//Verificar papeleria
					/*
						if(!DBFunctions.RecordExist("SELECT NUM_DOCUMENTO FROM DBXSCHEMA.MCONTROL_PAPELERIA WHERE TDOC_CODIGO='REM' AND FECHA_ANULACION IS NULL AND FECHA_USO IS NULL AND TIPO_DOCUMENTO='M' AND MAG_CODIGO="+agencia+" AND NUM_DOCUMENTO="+numDocumento+" ORDER BY NUM_DOCUMENTO FETCH FIRST 1 ROWS ONLY"))
						{
							errores+="No hay papeleria Encomienda/Giros disponible para Agencia:de la línea "+fila+". ";
							errLin=true;
					...
					*/
					...
					if(!errLin)
					{
						...
						if(ddlTipo.SelectedValue=="E")
						{
							...
							if(!chkAsignar.Checked)
							{
								sqlUpd.Add("INSERT INTO dbxschema.MENCOMIENDAS values('REM',"+numDocumento+",'E',"+agencia+",'"+ddlNitResponsable.SelectedValue+"',"+agenciaEntrega+",'"+responsableEntrega+"',"+planilla+","+linea+",'"+ruta+"','"+nitPlanillaManual+"',NULL,NULL,'"+nitPlanillaManual+"',NULL,NULL,'"+txtDescripcionEncomienda.Text+"',1,0,0,0,0,"+precio+",0,0,"+precio+",'"+fecha.ToString("yyyy-MM-dd")+"','"+DateTime.Now.ToString("yyyy-MM-dd")+"','E',NULL,'"+placa+"');");
								//Modificar papeleria(usada)
								sqlUpd.Add("UPDATE DBXSCHEMA.MCONTROL_PAPELERIA SET FECHA_USO='"+DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")+"',MPLA_CODIGO="+planilla+", TIPO_DOCUMENTO = 'E',MNIT_RESPONSABLE = '"+ddlNitResponsable.SelectedValue+"' WHERE TDOC_CODIGO='REM' AND NUM_DOCUMENTO="+numDocumento+";");
							}
							else
							{
						...
						else
						{
							...
							{             
								...
								//Modificar papeleria(usada)
								sqlUpd.Add("UPDATE DBXSCHEMA.MCONTROL_PAPELERIA SET FECHA_USO='"+DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")+"',MPLA_CODIGO="+planilla+", TIPO_DOCUMENTO = 'G',MNIT_RESPONSABLE = '"+ddlNitResponsable.SelectedValue+"' WHERE TDOC_CODIGO='REM' AND NUM_DOCUMENTO="+numDocumento+";");
						   }
			               else
                           {
				...
				fila++;
                if(dgrI.ItemType==ListItemType.Footer)
				{
					txtTotalEncomiendas.Text = totalEncomiendas.ToString("###,###,##0");
				}								
			}
			...
			*/
			...
			foreach(DataGridItem dgrI in dgrPagos.Items)
			{
				...
				//TextBox txtResponsableAnticipo=(TextBox)dgrI.FindControl("txtResponsableAnticipo");
				...
				if(txtNumPago.Text.Trim().Length>0 || ddlConceptoPago.SelectedValue.Length>0 || txtValorPago.Text.Trim().Length>0)
				{
                    ...
                    /* a partir de aca estaba comentado pero permitia meter anticipo aun no despachados a la agencia*/
					//Verificar papeleria
					if(!DBFunctions.RecordExist("SELECT NUM_DOCUMENTO FROM DBXSCHEMA.MCONTROL_PAPELERIA WHERE TDOC_CODIGO='ANT' AND FECHA_ANULACION IS NULL AND FECHA_USO IS NULL AND TIPO_DOCUMENTO='M'  AND MAG_CODIGO="+agencia+" AND NUM_DOCUMENTO="+numDocumento+" ORDER BY NUM_DOCUMENTO FETCH FIRST 1 ROWS ONLY"))
					{
						errores+="No hay papeleria de anticipo/servicio disponible para la Agencia de la línea "+fila+". ";
						errLin=true;
					}
                    /* hasta aca estaba comentado pero permitia meter anticipo aun no despachados a la agencia*/
					DataSet dsVerifica=new DataSet();
					string TipoDoc = "ANT";
					...
					if(!errLin)
					{
						...
						//Modificar papeleria(usada)
						sqlUpd.Add("UPDATE DBXSCHEMA.MCONTROL_PAPELERIA SET FECHA_USO='"+DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")+"',MPLA_CODIGO="+planilla+" ,MNIT_RESPONSABLE = '"+ddlPersonalAgencia.SelectedValue+"' WHERE TDOC_CODIGO='ANT' AND NUM_DOCUMENTO="+numDocumento+";");
			...
			if(errores.Length==0)
			{
				if(Opcion=="G")
				{
	                ...
	                string Programa = "Comercial.PlanillaManual";
					...
					if (DBFunctions.Transaction(sqlUpd))
						if(Tools.Browser.IsMobileBrowser())
							Response.Redirect(ConfigurationManager.AppSettings["MainMobileIndexPage"]+"?process=Comercial.PlanillaBus&pln="+planilla+ "&comando=" +Comando+"&programa="+Programa+"&act=4&path=Planillas Manuales");
						else
		...
		private void btnSeleccionar_Click(object sender, System.EventArgs e)
		{
			...
			*/
			...
			//Planilla
			planilla=DBFunctions.SingleData("Select NUM_DOCUMENTO from DBXSCHEMA.MCONTROL_PAPELERIA WHERE TDOC_CODIGO='PLA' AND TIPO_DOCUMENTO='M' AND FECHA_ANULACION IS NULL AND FECHA_USO IS NULL AND MAG_CODIGO="+agencia+" AND mnit_responsable = '"+nitTiqueteador+"' order by num_documento fetch first 1 rows only;");
			if(planilla.Length==0)
			{
				Response.Write("<script language='javascript'>alert('No hay planillas disponibles para la Agencia-Tiquetero.');</script>");
				return;
			}
