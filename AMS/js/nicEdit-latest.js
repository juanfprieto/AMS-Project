...
var nicEditorConfig = bkClass.extend({
	...
	iconsPath : '../js/nicEditIcons-latest.gif',
...
var nicUploadButton=nicEditorAdvancedButton.extend(
{
	...
	addPane: function()
	{
		...
		new bkElement("div").setStyle({fontSize:"14px",fontWeight:"bold",paddingBottom:"5px"}).setContent("Inserte una Imagen").appendTo(A);
	...
	uploadFile:function()
	{
		...
		C.open("POST", "../aspx/AMS.Tools.Upload.aspx", true);
		...
		//C.setRequestHeader("Content-Type", "multipart/form-data");  //xhr
	    C.setRequestHeader("X-File-Name", B.name);  //xhr
	    C.setRequestHeader("X-File-Size", B.size);  //xhr
	    C.setRequestHeader("X-File-Type", B.type);   //xhr
	    C.setRequestHeader("X-File-Width", "600");   //xhr de prueba
		C.send(A)
	},
	...
	{
		this.removePane();
		var link=B[0]; //.substr(0, B[0].indexOf("*"));

		if(!this.im)
		{
			this.ne.selectedInstance.restoreRng();
			var C="javascript:nicImTemp();";
			this.ne.nicCommand("insertImage",link);
			this.im=this.findElm("IMG","src",link)
		}
		...
		if(this.im)
		{
			//this.im.setAttributes({src:link,width:(A&&B.width)?Math.min(A,B.width):""})
			var w = B[1];
			this.im.setAttributes({src:link,width:(A&&w)?Math.min(A,w):""})
		}
	}
}
);
