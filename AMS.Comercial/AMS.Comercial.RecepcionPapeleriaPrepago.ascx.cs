namespace AMS.Comercial
{
	...
	/// <summary>
	///		Descripción breve de AMS_Comercial_RecepcionPapeleriaPrepago.
	/// </summary>
	public class AMS_Comercial_RecepcionPapeleriaPrepago : System.Web.UI.UserControl
	{
		#region Controles
		...
		private void btnRecibir_Click(object sender, System.EventArgs e)
		{
				...
				if(txtTalonarios.Text.Trim().Length>0 || txtInicioDocumento.Text.Trim().Length>0 || txtFinDocumento.Text.Trim().Length>0)
				{
					...
					if(!errLin)
					{
						/*for(int nD=inicio;nD<=fin;nD++)
							sqlUpd.Add("INSERT INTO DBXSCHEMA.MCONTROL_PAPELERIA VALUES('"+tipo+"',"+nD+",'"+clase+"',"+recepcion+",'"+DateTime.Now.ToString("yyyy-MM-dd")+"',NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL);");
						*/
						DBFunctions.NonQuery("call dbxschema.RECEPCION_PAPEL('TIQPRE',"+inicio+","+fin+",'M',"+recepcion+");");
			...
			if(errores.Length==0)
			{
				if(DBFunctions.Transaction(sqlUpd))
					Response.Redirect(indexPage+"?process=Comercial.RecepcionPapeleriaPrepago&act=1&path="+Request.QueryString["path"]+"&rec="+recepcion);
				else
