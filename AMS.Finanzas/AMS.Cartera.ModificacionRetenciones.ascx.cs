namespace AMS.Finanzas
{
	...
	/// <summary>
	///		Descripción breve de AMS_Cartera_ModificacionRetenciones.
	/// </summary>
	public partial class AMS_Cartera_ModificacionRetenciones : System.Web.UI.UserControl
	{
		#region Controles
		...
		protected void rblTipo_SelectedIndexChanged(object sender, System.EventArgs e)
		{
            ...
            string sqlRetenciones = "";
            if (Request.QueryString["pref"] != null && Request.QueryString["num"] != null)
                sqlRetenciones = ("SELECT DISTINCT(MFC." + prefi + ") " +
                "FROM  MFACTURA" + tabla + " MFC LEFT JOIN MFACTURA" + tabla + "RETENCION MFCR " +
                "ON MFCR." + prefi + "=MFC." + prefi + " AND MFCR." + nume + "=MFC." + nume + " WHERE " +
                "MFCR." + prefi + "='" + Request.QueryString["pref"] + "' AND " +
                "TVIG_VIGENCIA NOT IN ('P', 'C') order by MFC." + prefi + ";");
            else sqlRetenciones = ("SELECT DISTINCT(MFC."+prefi+") "+
                    "FROM MFACTURA" + tabla + " MFC LEFT JOIN MFACTURA" + tabla + "RETENCION MFCR " +
				    "ON MFCR."+prefi+"=MFC."+prefi+" AND MFCR."+nume+"=MFC."+nume+" AND "+
				    "TVIG_VIGENCIA NOT IN ('P', 'C') order by MFC."+prefi+";");
            bind.PutDatasIntoDropDownList(ddlPrefijo, sqlRetenciones);
		...
		private void CargarNumeros()
		{
            ...
            string sqlRetenciones = "";
            if (Request.QueryString["pref"] != null && Request.QueryString["num"] != null)
                 sqlRetenciones = ("SELECT MFC." + nume + " " +
                "FROM MFACTURA" + tabla + " MFC " +
                "WHERE MFC." + prefi + "='" + Request.QueryString["pref"] + "' AND " +
            ...
            bind.PutDatasIntoDropDownList(ddlNumero, sqlRetenciones);
     	}
		...
		protected void btnAceptar_Click(object sender, System.EventArgs e)
		{
			...
			arlSql.Add("UPDATE MFACTURA"+tabla+" SET MFAC_VALORETE="+totalR+" "+
				"WHERE "+prefi+"='"+ddlPrefijo.SelectedValue+"' AND "+nume+"="+ddlNumero.SelectedValue+";");
            // GUARDA REGISTRO en MHISTORIAL_CAMBIOS
            string tablaH = "MFACTURA"+tabla+" ";
            string detalle = ddlPrefijo.SelectedValue + ',' + ddlNumero.SelectedValue  + ", Modificó Retenciones";
            arlSql.Add("INSERT INTO MHISTORIAL_CAMBIOS VALUES(DEFAULT,'"+ tablaH+ "','U','"+ detalle+"','"+HttpContext.Current.User.Identity.Name.ToLower()+"','"+ DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "')");
