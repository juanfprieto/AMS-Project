			   ...
			   <IMG height="70" src="../img/AMS.Flyers.Nueva.png" border="0">
                ...
                <table id="Table" class="filtersIn">
                        ...
                        <tr>
							...
							<td colspan="3"><asp:dropdownlist id="ddlPrecios" class="dmediano" runat="server"></asp:dropdownlist></td>
						</tr>
						<tr>
							<td>Servicio :</td>
							<td colspan="3"><asp:dropdownlist id="servicio" class="dmediano" runat="server"></asp:dropdownlist>
                            <asp:Image id="bntLupa" runat="server"  ImageUrl="../img/infoLibro.jpg" onClick="listaPrecios()"></asp:Image>
                            </td>
...
<script type="text/javascript">
    ...
    // AQUI HAY QUE MOSTRAR LAS LISTAS DE PRECIOS DE TALLER CUANDO HAY MAS DE 1
    function listaPrecios(){
        var precio = document.getElementById("_ctl1_ddlPrecios").value;
        var servicio = '<%= servicio.ClientID %>'; 
        var sqlKit = 'call dbxschema.consultar_itemskit( (SELECT ppreTA_codigo FROM pPRECIOTALLER where ppreta_codigo = \'' + precio + '\' FETCH FIRST 1 ROWS ONLY),\'$\' ) ';
        ModalDialogCont('M', 'Elementos del KIT', servicio, sqlKit)
    }
</script>